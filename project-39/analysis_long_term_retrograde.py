...
DATA_FILE = Path("39-retrograde-market-volatility/sp500_data_1950.csv")
...
def analyze_volatility(df):
    ...
    metric = 'Realized_Vol_30d'
    print(f"\n--- Market Volatility ({metric}) Analysis (1950-Present) ---")
    
    # Baseline
    baseline_vol = df[metric].mean()
    print(f"Baseline Mean Volatility (N={len(df)}): {baseline_vol:.2f}%")
    
    # 1. Individual Planet Analysis
    for planet in PLANETS.keys():
        rx_col = f"{planet}_Rx"
        
        rx_data = df[df[rx_col] == 1][metric]
        direct_data = df[df[rx_col] == 0][metric]
        ...
        print(f"  Mean Vol (Rx):     {mean_rx:.2f}%")
        print(f"  Mean Vol (Direct): {mean_dir:.2f}%")
    ...
    for planet in PLANETS.keys():
        ...
        temp['Planet'] = planet
        plot_data.append(temp[[metric, 'Status', 'Planet']])
    
    viz_df = pd.concat(plot_data)
    
    sns.boxplot(x='Planet', y=metric, hue='Status', data=viz_df, showfliers=False)
    plt.title("S&P 500 Realized Volatility (30d) during Planetary Retrogrades (1950-Present)")
    plt.ylabel("Annualized Volatility % (Outliers Removed)")
    plt.grid(True, axis='y', alpha=0.3)
    
    output_path = OUTPUT_DIR / "retrograde_impact_long_term.png"
...
def main():
    ...
    df = df.dropna(subset=['Realized_Vol_30d']) 
    
    print(f"Loaded {len(df)} trading days.")
    
    # Calculate Retrogrades
    retro_df = get_retrograde_status(df['Date'])
    
    # Merge
    full_df = pd.concat([df.reset_index(drop=True), retro_df.reset_index(drop=True)], axis=1)
    
    # Analyze
    summary = analyze_volatility(full_df)
    
    # Save Results
    summary.to_csv(OUTPUT_DIR / "retrograde_analysis_summary_long_term.csv", index=False)
    # full_df.to_csv(OUTPUT_DIR / "sp500_with_retrogrades.csv", index=False) # Optional, can be large
